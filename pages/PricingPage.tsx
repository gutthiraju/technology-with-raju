...
import { CheckCircle2, ArrowRight, X, MapPin, Mail, Phone } from 'lucide-react';
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-20">
      <div className="text-center mb-16 space-y-4">
        ...
        <p className="text-xl text-slate-600 dark:text-slate-400 max-w-2xl mx-auto">Choose a plan that fits your learning pace. No hidden fees, just pure knowledge.</p>
      </div>

      <div className="grid md:grid-cols-3 gap-8">
        {plans.map((plan, i) => (
          <div key={i} className={`bg-white dark:bg-slate-900 p-8 rounded-3xl border ${plan.premium ? 'border-indigo-500 ring-4 ring-indigo-500/10 md:scale-105' : 'border-slate-200 dark:border-slate-800'} relative flex flex-col shadow-xl transition-transform hover:shadow-2xl`}>
            {plan.premium && (
      ...
      <div className="mt-20 bg-slate-100 dark:bg-slate-900/50 p-8 rounded-3xl text-center border border-slate-200 dark:border-slate-800">
        <h3 className="text-2xl font-bold mb-4 text-slate-900 dark:text-white">Bulk Access for Colleges?</h3>
        <p className="text-slate-600 dark:text-slate-400 mb-6">We offer special discounts for educational institutions and group purchases.</p>
        <button 
      ...
      {showEnterpriseModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          ></div>
          <div className="relative bg-white dark:bg-slate-900 w-full max-w-4xl rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col md:flex-row transform transition-all duration-300 scale-100 opacity-100">
            <button 
              onClick={() => setShowEnterpriseModal(false)}
              className="absolute top-6 right-6 z-10 p-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-500 dark:text-slate-300 rounded-full transition-all"
            >
              <X className="w-6 h-6" />
            </button>
            
            {/* Image Section */}
            <div className="md:w-1/2 relative h-48 md:h-auto min-h-[300px]">
              <a 
                href="" 
                target="_blank" 
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-slate-900/90 via-slate-900/40 to-transparent flex items-end p-8 pointer-events-none">
                <div className="text-white">
                  ...
                  <p className="text-xs text-slate-300 mt-1">Visit us in the heart of Bengaluru Tech Park</p>
                </div>
              </div>
            </div>

            {/* Info Section */}
            <div className="md:w-1/2 p-8 lg:p-12 space-y-8 bg-white dark:bg-slate-900 overflow-y-auto max-h-[80vh] md:max-h-full">
              <div>
              ...
              <div className="space-y-6">
                <div className="flex items-start gap-4">
                  <div className="bg-indigo-50 dark:bg-indigo-900/30 p-3 rounded-2xl shrink-0">
                ...
                <div className="flex items-start gap-4">
                  <div className="bg-violet-50 dark:bg-violet-900/30 p-3 rounded-2xl shrink-0">
                ...
                <div className="flex items-start gap-4">
                  <div className="bg-emerald-50 dark:bg-emerald-900/30 p-3 rounded-2xl shrink-0">
              ...
              <button 
                onClick={() => setShowEnterpriseModal(false)}
                className="w-full py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-2xl font-bold hover:opacity-90 transition-opacity flex items-center justify-center gap-2 shadow-xl shadow-slate-200 dark:shadow-none"
              >
                Close Details
              </button>
            </div>
